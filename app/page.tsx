...
export default async function HomePage() {
  ...
  const hasStories = (content.promotions || []).some((promotion) => promotion.enabled !== false);
  const publicNavigation = content.navigation.map((item) =>
    item.href === '#stories' ? { ...item, visible: item.visible && hasStories } : item
  );
  ...
  return (
    <main className="min-h-screen w-full overflow-x-hidden bg-white pt-[72px] font-sans text-white selection:bg-white selection:text-black md:bg-black md:pt-0">
      ...
      <Header brand={content.brand} navigation={publicNavigation} />
      ...
      <div className="bg-white px-2 pb-2 md:bg-black md:p-0">
        ...
        <StoriesCarousel promotions={content.promotions || []} />
      ...
      <div id="utility-start">
        ...
        <Footer brand={content.brand} navigation={publicNavigation} />
      </div>
    </main>
  );
}
